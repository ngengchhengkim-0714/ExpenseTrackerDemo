...
RSpec.describe 'Transaction Management', type: :system do
  ...
  describe 'Viewing transactions' do
    let!(:expense) { create(:transaction, user: user, category: category_expense, description: 'Coffee shop', amount: 5.50, transaction_type: 'expense', date: Date.today) }
  ...
  describe 'Date range filtering' do
    ...
    let!(:recent_transaction) { create(:transaction, user: user, date: Date.today, description: 'Recent transaction') }
